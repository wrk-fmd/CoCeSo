...
<dl class="list-spacing dl-horizontal">
  ...
  <dd <c:if test="${incident.blue}">class="blue"</c:if>>
    <c:choose>
      ...
      <c:otherwise>
        <spring:message code="label.incident.type.${fn:toLowerCase(incident.type)}" text="${incident.type}"/>
      </c:otherwise>
    </c:choose>
  </dd>
  ...
  <c:if test="${not empty incident.caller}">
    <dt><spring:message code="label.incident.caller"/></dt>
    <dd><c:out value="${incident.caller}"/></dd>
  </c:if>

  <c:if test="${not empty incident.casusNr}">
    <dt><spring:message code="label.incident.casus"/></dt>
    <dd><c:out value="${incident.casusNr}"/></dd>
  </c:if>

  <dt><spring:message code="label.incident.state"/></dt>
  <dd><spring:message code="label.incident.state.${fn:toLowerCase(incident.state)}" text="${incident.state}"/></dd>
</dl>
