...
<span <c:if test="${incident.blue}">class="blue"</c:if>>
  <c:choose>
    <c:when test="${incident.type == 'Task'}">
      <c:choose>
        <c:when test="${incident.blue}"><spring:message code="label.incident.stype.task.blue"/>:</c:when>
        <c:otherwise><spring:message code="label.incident.stype.task"/>:</c:otherwise>
      </c:choose>
    </c:when>
    <c:when test="${incident.type == 'HoldPosition'}"><span class="glyphicon glyphicon-record"></span>:</c:when>
    <c:when test="${incident.type == 'Standby'}"><span class="glyphicon glyphicon-pause"></span>:</c:when>
    <c:otherwise>
      <spring:message code="label.incident.stype.${fn:toLowerCase(incident.type)}" text="${incident.type}"/>:
    </c:otherwise>
  </c:choose>
  <c:choose>
    <c:when test="${incident.type == 'Task' || incident.type == 'Transport'}">
      <c:choose>
