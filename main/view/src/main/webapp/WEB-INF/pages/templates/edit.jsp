 ...
 */
--%>
<%@page import="at.wrk.coceso.entity.enums.UnitType"%>
<script type="text/html" id="template-unit-row">
  ...
  <td>
    <p data-bind="css: type.formcss">
      <select class="form-control" data-bind="value: type, attr: {form: 'table_form_' + id}">
        <option value=""><spring:message code="unit.type"/>...</option>
        <c:forEach items="<%=UnitType.values()%>" var="type">
          <option value="${type}"><spring:message code="unit.type.${type}" text="${type}"/></option>
        </c:forEach>
      </select>
    </p>
  ...
  <td data-bind="css: section.formcss, if: $root.concern.hasSections">
    <select class="form-control" data-bind="value: section, options: $root.concern.selectSections,
            optionsValue: 'value', optionsText: 'name', attr: {form: 'table_form_' + id}">
    </select>
  </td>
  ...
  <td data-bind="css: home.formcss">
    <textarea rows="3" class="form-control" placeholder="${home}" data-bind="value: home, valueUpdate: 'input', attr: {form: 'table_form_' + id}, point: true"></textarea>
  </td>
</script>
